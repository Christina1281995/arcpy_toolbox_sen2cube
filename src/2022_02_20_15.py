...
def get_points(aoi_in):
    ...
    # Get current environment and active map
    ...
    # in standard web mercator left, bottom, right, top, nan, nan, nan,nan
    # p_lowleft
    points.append(arcpy.Point(float(individual_units[0]), float(individual_units[1])))
    # p_lowright
    points.append(arcpy.Point(float(individual_units[2]), float(individual_units[1])))
    # p_upperright
    points.append(arcpy.Point(float(individual_units[2]), float(individual_units[3])))
    # p_upperleft
    points.append(arcpy.Point(float(individual_units[0]), float(individual_units[3])))
...
if __name__ == '__main__':
    ...
    # icon
    # raw_data = urlopen("https://github.com/Christina1281995/arcpy_toolbox_sen2cube/blob/main/src/Picture1.png").read()
    # image = ImageTk.PhotoImage(data=raw_data)
    # popup.tk.call('wm', 'iconphoto', popup._w, image)
    ...
    while status != "SUCCEEDED":
        ...
        if elapsed_time > 200 and refresh == False:
            # refresh
            token_text = refresh_token(token_text, auth_token_url, auth_client_id)
