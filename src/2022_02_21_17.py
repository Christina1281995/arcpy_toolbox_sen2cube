...
Required Arguments: <parameter0> Area of Interest
                    ...
                    <parameter4> End Date
                    <parameter5> Comment
                    <parameter6> Favourite
                    <parameter7> Output Directory
...
def fetch_token(username: str, password: str,
    ...
    try:
        ...
        arcpy.AddErrorMessage(f"Unknown error on authentication for token url {auth_token_url} as client {auth_client_id}.",
            exc_info=True, )
...
def refresh_token(token: OAuth2Token, auth_token_url: str, auth_client_id: str) -> OAuth2Token:
  try:
    ...
    arcpy.AddError(f"Authorisation failed for token url {auth_token_url} as client {auth_client_id}.", exc_info=True,)
  except InvalidGrantError as e:
...
def get_points(aoi_in):
    ...
    filename = "aoi.shp"
    filename_calc = "aoi_calc.shp"

    #create polygons
    poly = arcpy.Polygon(polyArray, spatial_reference)
    poly_proj =  poly.projectAs(arcpy.SpatialReference(4326))

    #write WGS84 aoi for visualization
    arcpy.CreateFeatureclass_management(out_dir, filename, "POLYGON", spatial_reference = arcpy.SpatialReference(4326))
    data_path = os.path.join(str(out_dir), filename)
    cur = arcpy.da.InsertCursor(data_path, "SHAPE@")
    cur.insertRow([poly_proj])

    #reproject for area calculation
    outCRS = arcpy.SpatialReference(3416)
    data_path_calc = os.path.join(str(out_dir), filename_calc)
    arcpy.Project_management(data_path, data_path_calc, outCRS)
    for row in arcpy.da.SearchCursor(data_path_calc, "SHAPE@AREA"):
        arcpy.AddMessage(row[0])
        area = row[0]/1000000

    if area < 50:
        #Add layer to map
        activeMap.addDataFromPath(data_path)
        #delete calculation layer
        arcpy.Delete_management(data_path_calc)
    else:
        arcpy.AddMessage("The area was too big, process is cancelled")
        arcpy.AddError(f"The AOI is too big with {area} km2, please select an area with max size 50km2")
        sys.exit(0)
...
if __name__ == '__main__':
    ...
    # TODO get icon from web url
    
    URL = "https://raw.githubusercontent.com/Christina1281995/arcpy_toolbox_sen2cube/main/src/images_files/sen2image%20(3).ico"
    u = urlopen(URL)
    raw_data = u.read()
    u.close()
    im = Image.open(BytesIO(raw_data))
    photo = ImageTk.PhotoImage(im)
    popup.tk.call('wm', 'iconphoto', popup._w, photo)
    ...
    def enter(event):
        ...
        # arcpy.AddMessage(f"Login time: {str(login_time)}")
    ...
    # Bind Enter key to the "enter()" function
    ...
    aoi = "{\"type\":\"FeatureCollection\",\"features\":[{\"type\":\"Feature\",\"properties\":{},\"geometry\":{\"type\":\"Polygon\",\"coordinates\":[[[" + \
          str(aoi_points[0].firstPoint.X) + "," + str(aoi_points[0].firstPoint.Y) + "],[" + str(aoi_points[1].firstPoint.X) + "," + str(aoi_points[1].firstPoint.Y) + "],[" + \
          str(aoi_points[2].firstPoint.X) + "," + str(aoi_points[2].firstPoint.Y) + "],[" + str(aoi_points[3].firstPoint.X) + "," + str(aoi_points[3].firstPoint.Y) + "],[" + \
    ...
    favourite = arcpy.GetParameter(6)
    if favourite == True:
    ...
    # arcpy.AddMessage(f"Token_text now is: {token_text}")
    
    while status != "SUCCEEDED":
        
        # Check time to refresh token
        current_time = time.time()
        #arcpy.AddMessage(f"Current time: {str(current_time)}")
        elapsed_time = current_time - login_time
        #arcpy.AddMessage(f"Elapsed time: {str(elapsed_time)}")
        if elapsed_time > 200 and refresh == False:
            # refresh
            refresh_token_text = refresh_token(token_text, auth_token_url, auth_client_id)
            arcpy.AddMessage("Token has been refreshed.")
            refresh = True

        # arcpy.AddMessage(f"refresh_token_text is: {refresh_token_text}")
